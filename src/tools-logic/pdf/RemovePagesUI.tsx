...
import { Scissors, Loader2, Download, Trash2, AlertCircle } from "lucide-react";
...
export default function RemovePagesUI() {
  ...
  const process = async () => {
    ...
    if (targets.length === 0) {
      setError("Enter at least one valid page number, for example 1,3,5-7.");
      return;
    }
    if (targets.length >= pageCount) {
      setError("You cannot remove every page — a PDF needs at least one page.");
      return;
    }
    ...
    try {
      ...
      // the indices of the pages still queued for removal.
      for (const index of [...targets].reverse()) doc.removePage(index);

      const bytes = await doc.save();
      const blob = new Blob([bytes as unknown as BlobPart], { type: "application/pdf" });
      downloadFileBlob(blob, `${baseName(file.name)}_pages_removed.pdf`);
      window.dispatchEvent(
        new CustomEvent("tool_processed", { detail: { fileName: file.name } })
      );
    } catch (err) {
      setError(err instanceof Error ? err.message : "Something went wrong while editing the PDF.");
    } finally {
      setIsProcessing(false);
    }
  };
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Use commas for single pages and a dash for ranges. Everything runs in your browser.
            </p>
          </div>
