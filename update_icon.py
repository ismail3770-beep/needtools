...
with open('src/config/categories.ts', 'r', encoding='utf-8') as f:
    cat_content = f.read()

cat_content = cat_content.replace('iconName: "FileText",', 'iconName: "CustomPdf",')

with open('src/config/categories.ts', 'w', encoding='utf-8') as f:
    f.write(cat_content)

# Update DynamicIcon.tsx
with open('src/components/icons/DynamicIcon.tsx', 'r', encoding='utf-8') as f:
    dyn_content = f.read()
...
if 'CustomPdf' not in dyn_content:
    dyn_content = dyn_content.replace(
        "if (name.startsWith('Fc')) {",
        custom_pdf_code + "\n  if (name.startsWith('Fc')) {"
    )

    with open('src/components/icons/DynamicIcon.tsx', 'w', encoding='utf-8') as f:
        f.write(dyn_content)
